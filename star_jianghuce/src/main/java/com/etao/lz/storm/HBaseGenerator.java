...
/**
 * HBase数据流生成类，支持吸星大法中的流量日志和业务日志相关表的读取
 * 
 * @author jiuling.ypf
 * 
 */
public class HBaseGenerator implements java.io.Serializable {
	...
	// 序列化ID号
	private static final long serialVersionUID = 5411874877370668738L;
	 ...
	 * @param endTimestamp
	 */
	public HBaseGenerator(String tableName, int startTimestamp, int endTimestamp) {
	...
	public ResultScanner scanMainTable(short shardingKey, byte[] lastKey) {
		...
		if (shardingKey >= 0 && shardingKey < MbConstants.XXDF_HBASE_SHARDING_NUM) { // 按照rowkey范围扫描
			...
			//		endTs = (endTs / Constants.XXDF_HBASE_TIME_INTERVAL + 1)
				//			* Constants.XXDF_HBASE_TIME_INTERVAL;
				  int tmpTs = endTs + MbConstants.XXDF_HBASE_TIME_INTERVAL / 2;
				    endTs = tmpTs - tmpTs % MbConstants.XXDF_HBASE_TIME_INTERVAL;
				}
		...
		// 记录最近一次扫描到的时间戳
		if(delay == 0 )       //保证了延迟时，时间窗口不会滑到当前时间之前
		lastTimestamp = endTs;
		return rs;
	}
