...
/**
 * 江湖策 实时成交增量计算 Storm 任务总入口
 * 
 * 执行方式：
 * <code>star-flow-bigb jar <packaged-code.jar> com.etao.lz.storm.JhcDeltaTopology star_jhc_delta</code>
 * 
 * 停止方式： <code>storm kill star_jhc_delta</code>
 * 
 * @author yuanhong.shx
 * 
 */
public class JhcDeltaTopology {

	public static void main(String[] args) throws Exception {
		TopologyBuilder builder = new TopologyBuilder();
		PropConfig pc = new PropConfig("delta_conf.properties");

		int topoWorkers = Integer.valueOf(pc.getProperty("topo_workers"));

		int businessSpoutTasks = Integer.valueOf(pc.getProperty("bspout_num"));
		builder.setSpout(MbConstants.BUSINESS_SPOUT_ID, new BusinessSpout(),
				businessSpoutTasks);
		int trafficSpoutTasks = Integer.valueOf(pc.getProperty("tspout_num"));
		builder.setSpout(MbConstants.TRAFFIC_SPOUT_ID, new TrafficSpout(),
				trafficSpoutTasks);

		int detailBoltTasks = Integer.valueOf(pc.getProperty("detail_num"));
		builder.setBolt(MbConstants.DETAIL_BOLT_ID, new DeltaDetailBolt(),
				detailBoltTasks)
		...
		conf.put(Config.TOPOLOGY_WORKER_CHILDOPTS, "-Xms6000M -Xmx6000M");
		...
		conf.put("monitorInterval",
				Integer.valueOf(pc.getProperty("monitor_interval")));
